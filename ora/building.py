...
Задача.
Выбрать из бигтабла жилые здания (по расширенным данным: '99:44110000')
и загрузить в фичекласс building
Отметить в статусе:
	select * from MKV.bigtab where xdata like '%99:44110000%' ;
	update MKV.bigtab set stats='загружено в FC; тест'
	where xdata like '%99:44110000%' ;

Куда-откуда:
mkv.building.geom
	mkv.bigtab.coords (поворотные точки)
	берем точки х, у и преобразуем в WKT POLYGON (примеры c:\d\code\shape\shp2sql.py)

Insert into mkv.building (GEOM
	) values (SDO_GEOMETRY('POLYGON ((30 10, 10 20, 20 40, 40 40, 30 10))', 82353) ) ;
...
примеры cx_Oracle:
c:\Python25\cx_Oracle-doc\samples\
...
class VoraDriver:
	def __init__(self):
		''' If only one parameter is passed, a connect string is assumed which is to be of the format
		user/password@dsn
		'''
...
def doWork(inp=''):
	''' после загрузки, надо починять геометрию
select * from (
...
order by fid
	...
	for recnum,rec in zip(range(len(selected)),selected):
		...
		ora.cursor.execute("""Insert into mkv.building (GEOM
			) values (
