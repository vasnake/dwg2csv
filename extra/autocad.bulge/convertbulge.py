...
'''
Testing algorithms for conversion from AutoCAD Bulge to polyline
'''
...
class AutoLISP:
    ...
    AutoCAD commands used for debug
...
def testBulge(x1, y1, x2, y2, bulge, sublen=0.0):
    ''' Converts AutoCAD polyline segment with bulge to arc (radius, center, angles, start-stop points)
    ...
    print 'testBulge done [%s; %s; %s; %s; %s; %s; %s; %s; %s; subpoints %s]' % (
...
def main():
    ...
    #~ грузить в Оракл
    #~ визуализировать выборку в Автокад
